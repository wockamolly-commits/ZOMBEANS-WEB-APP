...
export async function updateOperationsProfile(
  _previous: OperationsProfileState,
  formData: FormData
): Promise<OperationsProfileState> {
  const { profile: actor } = await requireStaff("/workspace/account");
  ...
  if (changed) {
    ...
    if (updated.error || !updated.data) {
      console.error(
        "[account] operations profile update failed:",
    ...
    if (user.data.user) {
      ...
      if (metadataUpdate.error) {
        console.error(
          "[account] operations auth metadata update failed:",
          metadataUpdate.error.message
        );
      }
    } else if (user.error) {
      console.error("[account] operations auth lookup failed:", user.error.message);
    }

    const audit = await admin.from("audit_logs").insert({
      actor_profile_id: actor.id,
      action: "operations_profile.updated",
    ...
    if (audit.error) {
      console.error("[account] operations profile audit failed:", audit.error.message);
    }
  }
