...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.rpc(
    "customer_acknowledge_rider_arrival",
    {
      p_code: shortCode,
    }
  );
  ...
  if (error) {
    console.error("[customer] rider arrival acknowledgement failed:", error);
  ...
  if (typeof data !== "string" || data.length === 0) {
    return NextResponse.json(
      { error: "Rider arrival is not ready for acknowledgement." },
      { status: 409 }
    );
  }
