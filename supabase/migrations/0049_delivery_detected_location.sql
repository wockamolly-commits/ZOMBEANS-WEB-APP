-- Delivery orders now carry TWO pieces of location information:
--   1. The delivery address (manual/saved/map-picked) — street/barangay/city +
--      lat/lng, which stay authoritative for the fee and for "deliver here".
--   2. The device's auto-detected GPS reading at checkout — detected_lat/lng +
--      detected_address — captured independently so it is available even when a
--      saved address (with its own coordinates) is chosen. Used only to help the
--      rider navigate; it never affects pricing.

alter table delivery_addresses
  add column if not exists detected_lat numeric(10,7),
  add column if not exists detected_lng numeric(10,7),
  add column if not exists detected_address text;

-- Redefinition of place_order matching migration 0047, with the delivery_addresses
-- insert extended to persist the auto-detected GPS reading alongside the address.
...
begin
  ...
  if v_service_mode = 'delivery' then
    insert into delivery_addresses (
      ...
      detected_lat, detected_lng, detected_address    -- GPS: device reading at checkout
    ) values (
      v_order_id,
      ...
      (v_delivery->>'detected_lat')::numeric,          -- GPS
      (v_delivery->>'detected_lng')::numeric,          -- GPS
      nullif(v_delivery->>'detected_address', '')      -- GPS
    );
  end if;
