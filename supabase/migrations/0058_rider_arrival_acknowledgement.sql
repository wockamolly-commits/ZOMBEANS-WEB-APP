-- Persist customer acknowledgement for rider-arrival alerts so the looping
-- delivery arrival alarm remains stopped after refreshes.

alter table rider_assignments
  add column if not exists customer_arrival_acknowledged_at timestamptz;

create or replace function customer_acknowledge_rider_arrival(p_code text)
returns timestamptz
language plpgsql
security definer
...
begin
  ...
  if v_order_id is null then
    select max(ra.customer_arrival_acknowledged_at)
  ...
  set customer_arrival_acknowledged_at = coalesce(
    customer_arrival_acknowledged_at,
    now()
  )
  where order_id = v_order_id
    and arrived_at is not null
  returning customer_arrival_acknowledged_at into v_acknowledged_at;
  ...
  where ra.order_id = v_order_id;

  return v_acknowledged_at;
end;
$$;

revoke all on function customer_acknowledge_rider_arrival(text) from public;
grant execute on function customer_acknowledge_rider_arrival(text) to anon, authenticated;
