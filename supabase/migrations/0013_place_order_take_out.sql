-- place_order() — drop the dine-in table requirement and support take_out.
-- The cafe does not use numbered tables, so dine_in no longer attaches a
-- table and never raises MISSING_TABLE/INVALID_TABLE. take_out needs no
-- mode-specific fields (just name + phone), so it falls through the
-- service-mode branch. pickup and delivery are unchanged.
...
begin
  ...
  --    dine_in and take_out are both counter-based and need no extra fields.
